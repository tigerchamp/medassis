...
async function getElders(req, res) {
  try {
    ...
    const [elders] = await getPool().query(
      'SELECT * FROM elders WHERE family_id = ? ORDER BY created_at DESC',
      [familyId]
    );
...
async function addElder(req, res) {
  try {
    const familyId = req.familyId;
    const { name, gender, age, bloodType, allergies, conditions, phone, avatar } = req.body;
    ...
    await getPool().query(
      `INSERT INTO elders (id, family_id, name, gender, age, blood_type, allergies, conditions, phone, avatar)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, familyId, name, gender || '男', age || 0, bloodType || null, allergies || null, conditions || null, phone || null, elderAvatar]
    );
